...
const TermsOfServiceBackgroundEditModal = ({ Refetch, TermsOfServiceData }) => {
  ...
  // Set Image Preview
  useEffect(() => {
    if (TermsOfServiceData?.background) {
      setPreview(TermsOfServiceData.background);
    }
  }, [TermsOfServiceData]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Close modal
      document.getElementById("Terms_Of_Service_Background_Edit_Modal").close();

      Swal.fire({
        icon: "success",
        title: "Background Edited Successfully",
        text: "Our Mission Background has been successfully Edited.",
        showConfirmButton: false,
        timer: 1000,
      });
    } catch (error) {
      console.error("Edit Failed:", error);
      setModalError("Something went wrong while Edited the data. Try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="modal-box max-w-5xl w-full p-0 bg-gradient-to-b from-white to-gray-100 text-black">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-5">
        ...
        <div
          ...
          className={`w-full h-[500px] border-2 border-dashed ${
...
// Prop Validation
TermsOfServiceBackgroundEditModal.propTypes = {
...
export default TermsOfServiceBackgroundEditModal;
