...
const AllTrainerBookingAccepted = ({ AllTrainerBookingAcceptedData }) => {
  ...
  // Normalize search terms to lowercase for consistent filtering
  ...
  const sessionOptions = useMemo(() => {
    const uniqueSessions = new Set(
      AllTrainerBookingAcceptedData.map((b) => b.sessions.length)
    );
  ...
  const durationOptions = useMemo(() => {
    const uniqueDurations = new Set(
      AllTrainerBookingAcceptedData.map((b) => b.durationWeeks)
    );
  ...
  const monthYearOptions = useMemo(() => {
    ...
    AllTrainerBookingAcceptedData.forEach(({ bookedAt }) => {
      if (!bookedAt) return;
  ...
  const filteredData = useMemo(() => {
    return AllTrainerBookingAcceptedData.filter((booking) => {
