...
// Import Utility
// import { isBookingInMonthYear } from "../../../../Utility/dateFilters";
...
const AllTrainerBookingCompleted = ({ AllTrainerBookingCompletedData }) => {
  ...
  const sessionOptions = useMemo(() => {
    const uniqueSessions = new Set(
      AllTrainerBookingCompletedData.map((b) => b.sessions.length)
    );
  ...
  const durationOptions = useMemo(() => {
    const uniqueDurations = new Set(
      AllTrainerBookingCompletedData.map((b) => b.durationWeeks)
    );
  ...
  const monthYearOptions = useMemo(() => {
    ...
    AllTrainerBookingCompletedData.forEach(({ bookedAt }) => {
      if (!bookedAt) return;
  ...
  const filteredData = useMemo(() => {
    return AllTrainerBookingCompletedData.filter((booking) => {
    ...
    <>
      ...
      <div className="bg-gray-400 py-2">
        <h3 className="font-semibold text-white text-center text-lg">
          All Trainer Bookings Completed
        </h3>
      </div>
