...
const AllCompleatInvoices = ({ CompletedTierPaymentData }) => {
  ...
  const tierOptions = useMemo(() => {
    return [...new Set(CompletedTierPaymentData?.map((item) => item.tier))];
  }, [CompletedTierPaymentData]);

  // Get unique duration options for filter dropdown
  const durationOptions = useMemo(() => {
    return [...new Set(CompletedTierPaymentData?.map((item) => item.duration))];
  }, [CompletedTierPaymentData]);

  // Filtered Data Logic
  const filteredData = useMemo(() => {
    if (!CompletedTierPaymentData) return [];

    return CompletedTierPaymentData.filter((item) => {
