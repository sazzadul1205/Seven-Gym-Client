...
const ViewNotesModal = ({ Notes, refetch }) => {
  const { user } = useAuth();
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axiosPublic.delete("/Schedule/DeleteNote", {
        data: { email, noteID: Notes.id },
      });

      document.getElementById("View_Notes_Modal")?.close();
      setShowDeleteConfirm(false);
      refetch();
    } catch (error) {
    ...
    <div className="modal-box p-0 relative">
      ...
      <div className="flex justify-between items-center border-b border-gray-300 px-4 py-3">
        <h3 className="font-bold text-2xl">
          Note: <span className="text-blue-600">{Notes?.title}</span>
        </h3>
        <ImCross
          className="hover:text-[#F72C5B] cursor-pointer transition duration-200"
          onClick={() => document.getElementById("View_Notes_Modal")?.close()}
        />
      </div>
      ...
      <div className="flex items-center gap-2 mb-4 bg-red-200 px-5 py-2">
        ...
        <p className="text-xl">
          {Notes?.isImportant ? "High Importance" : "Normal Notes"}
        </p>
      </div>
      ...
      <div className="space-y-4 py-4 px-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p>{new Date(Notes?.reminder).toLocaleString()}</p>
          </div>
        </div>
